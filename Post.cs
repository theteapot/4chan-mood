...
namespace WebAPIClient
{
    class Post
    {
        ...
        public Post(string postNumber, string postBody)
        {
        ...
        {
            decimal badWords = WordFilter();
            int length = PostBody.Length == 0 ? 1 : PostBody.Length;
            decimal score = (0m + badWords) / length;
            PostScore = score;
            return score;
        }

        private decimal WordFilter()
        {
            decimal badWords = 0m;
        ...
        public override string ToString() => $"{PostNumber}: {PostBody}";
